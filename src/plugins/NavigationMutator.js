 ...
 * INSTALLATION
 * ------------
 * To install the plugin, simply import this file in the docusaurus.config.js file as follows (changing the path if required):
 * 
 * module.exports = {
 *   ...
 *   clientModules: [
...
class NavigationMutator {
  ...
  init() {
    ...
    if(this.ready){
      if($("#nav-level-3-styles").length == 0) {
        $("body").append(`<style id="nav-level-3-styles" type="text/css">${this.STYLES}</style>`); // inject CSS styles if not present
      }
      this.wait();
    }
  }
  ...
  mutate() {
    ...
    $(`${mutator.SIDEBAR_EXPANDABLE_LINK_SELECTOR}:not([${mutator.MUTATED_FLAG}])`).each(function() {
      ...
      if(location().endsWith(linkL1.attr("href"))) {
